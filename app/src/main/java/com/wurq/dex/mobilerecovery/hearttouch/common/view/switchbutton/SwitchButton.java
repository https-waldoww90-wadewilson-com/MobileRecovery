package com.wurq.dex.mobilerecovery.hearttouch.common.view.switchbutton;
...
public class SwitchButton extends CompoundButton {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        offsetX = mSafeRectF.width() - BETA;
        // thumb
        mPresentThumbRectF.set(mThumbRectF);
        if (mIsThumbUseDrawable) {
            if (isPressed()) { // TODO: 16/8/31 项目修改
                if (isChecked()) {
                    mPresentThumbRectF.left -= BETA;
                    mPresentThumbRectF.offset(mProcess * offsetX + BETA, 0);
                } else {
                    mPresentThumbRectF.right += BETA;
                    mPresentThumbRectF.offset(mProcess * offsetX, 0);
                }
...
//                offsetX = mSafeRectF.width() - BETA;
            ...
            case MotionEvent.ACTION_MOVE:
                float x = event.getX();
                setProcess(getProcess() + (x - mLastX) / offsetX);
                mLastX = x;
                break;
